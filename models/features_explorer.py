...
class ModelVisualizer:
    ...
    def _apply_dimension_reduction(self, data, method='pca', n_components=2, perplexity=30, n_neighbors=15, min_dist=0.1, metric='euclidean'):
        """
        Apply dimension reduction technique to the data.
    ...
    def create_visualizations(self, save_to_file=True, show_labels=False, not_showing_building_labels=False, use_tsne=False, use_umap=False):
        """
        Create all visualizations for the collected features.
    ...
    def _visualize_encoders(self, save_to_file=True, show_labels=False, not_showing_building_labels=False, use_tsne=False, use_umap=False):
        """
        Visualize the SAR encoder output space with different dimensionality reduction techniques.
    ...
    def _visualize_projection_space(self, save_to_file=True, show_labels=False, not_showing_building_labels=False, use_tsne=False, use_umap=False):
        """
        Visualize the SAR and optical projection space with different dimensionality reduction techniques.
...
def main():
    ...
    parser.add_argument("--use_tsne", action="store_true", help="Use t-SNE for visualization")
    # using umap or not
    parser.add_argument("--use_umap", action="store_true", help="Use UMAP for visualization")
    ...
    # Set UMAP parameters if specified
    if args.use_umap:
        visualizer._apply_dimension_reduction = lambda data, method='pca', n_components=2, perplexity=30, n_neighbors=args.umap_n_neighbors, min_dist=args.umap_min_dist, metric=args.umap_metric: \
            ModelVisualizer._apply_dimension_reduction(visualizer, data, method, n_components, perplexity, n_neighbors, min_dist, metric)
