...
class SimpleDatasetLoader:
    ...
    def __init__(self, root_dir, split='train', limit=None):
        ...
        self.image_ids = [f.name.replace('_post_disaster.tif', '') 
                          for f in post_event_dir.glob('*_post_disaster.tif')][:limit]
    ...
    def load_tiff_data(self, path):
        ...
        with rasterio.open(path) as data_reader:
            if data_reader.count > 1:
                # Multi-band image
                return np.stack([data_reader.read(i+1) for i in range(min(3, data_reader.count))], axis=2)
            else:
                # Single-band image
...
class PatchExtractor:
    ...
    def extract_patches(self, pre_img, post_img, label):
        ...
        for y in range(0, h - self.patch_size + 1, self.stride):
            for x in range(0, w - self.patch_size + 1, self.stride):
                # Extract patches
                pre_patch = pre_img[y:y+self.patch_size, x:x+self.patch_size].copy()
                post_patch = post_img[y:y+self.patch_size, x:x+self.patch_size].copy()
                label_patch = label[y:y+self.patch_size, x:x+self.patch_size].copy()
                
                # Check if patch is valid (contains enough valid pixels)
                valid_mask = ~np.isnan(pre_patch).any(axis=2) & ~np.isnan(post_patch).any(axis=2)
    ...
    def create_balanced_dataset(self, patches):
        ...
        logger.info(f"  - Found {len(pos_patches)} positive and {len(neg_patches)} negative patches")
        
        # Determine samples to keep for balance
        total_samples = min(len(pos_patches) + len(neg_patches), self.max_pairs_per_image)
...
def split_train_val(all_patches, train_ratio=0.8, balance_ratio=0.5, random_seed=42):
    ...
    # Log statistics
    logger.info(f"Split dataset into {len(train_patches)} training and {len(val_patches)} validation patches")
...
def save_patches_hdf5(output_dir, split, patches, config):
    ...
    logger.info(f"Saved {total_patches} patches ({pos_count} positive, {neg_count} negative)")
    return h5_path


def main():
    parser = argparse.ArgumentParser(description='Extract and save patches for contrastive learning')
    parser.add_argument('--config', type=str, default='configs/default.yaml',
                        help='Path to configuration file')
    parser.add_argument('--output_dir', type=str, default='data/processed_patches',
